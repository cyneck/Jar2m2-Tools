...
namespace WinFormJar2m2
{
    public partial class Form1 : Form
    {
        ...
        private void button_remote_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(this.textBoxRepoId.Text))
            {
                MessageBox.Show("repositoryId不能为空");
                return;
            }
            ...
            string strInput = "mvn deploy:deploy-file " +
                " -DgroupId=" + this.textBoxGroupId.Text +
                " -DartifactId=" + this.textBoxArtifactId.Text +
                " -Dversion=" + this.textBoxVersion.Text +
                " -Dpackaging=jar" +
                " -Dfile=" + this.textBoxFilePath.Text +
                " -Durl=[url]" + this.textBoxUrl+
                " -DrepositoryId=[id]"+this.textBoxRepoId;
            //向cmd窗口发送输入信息
            p.StandardInput.WriteLine(strInput + "&exit");
